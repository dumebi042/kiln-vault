...
// ── Fuzz: deposit → redeem round-trip with varying amounts ──

contract FuzzRoundTrip is FuzzBase {
    /// @dev Fuzz: deposit amount varies across ranges
    function testFuzz_depositRedeemRoundTrip(uint256 amount) public {
        _deploy(6, 6, 0, 0);
        vm.assume(amount >= 10 ** 12 && amount <= 10 ** 15); // meaningful range
        ...
        vm.prank(USER1);
        uint256 redeemed = vault.redeem(shares, USER1, USER1);

        // Floor rounding means we lose at most 1 wei
        assertApproxEqAbs(
            redeemed,
            amount,
            1,
            "redeem <= deposit within 1 wei"
        );
    }
}

// ── Fuzz: preview consistency under varying fees ──

contract FuzzPreviewConsistency is FuzzBase {
    function testFuzz_previewDeposit(uint256 amount, uint256 fee) public {
        fee = bound(fee, 0, 30 * 10 ** 6); // 0-30%
        _deploy(6, 6, fee, 0);
        _depositUser(USER1, 100_000 * 10 ** 6); // seed

        amount = bound(amount, 10 ** 6, 1_000_000 * 10 ** 6);
        uint256 p = vault.previewDeposit(amount);
        asset.mint(USER2, amount);
        vm.startPrank(USER2);
        asset.approve(address(vault), amount);
        uint256 e = vault.deposit(amount, USER2);
        vm.stopPrank();
        assertEq(p, e, "previewDeposit == deposit");
    }

    function testFuzz_previewRedeem(uint256 shares) public {
        _deploy(6, 6, 0, 0);
        _depositUser(USER1, 100_000 * 10 ** 6);
        shares = bound(shares, 10 ** 6, vault.balanceOf(USER1));
        uint256 p = vault.previewRedeem(shares);
        vm.prank(USER1);
        uint256 e = vault.redeem(shares, USER1, USER1);
        assertEq(p, e, "previewRedeem == redeem");
    }
...
// ── Fuzz: offset and decimal edge cases ──

contract FuzzDecimals is FuzzBase {
    function testFuzz_decimalsAndOffset(uint8 dec, uint8 offset) public {
        offset = uint8(bound(offset, 0, 23));
        dec = uint8(bound(dec, 0, 18));
        if (dec == 1 || dec == 0) return; // skip unusual but valid
        _deploy(dec, offset, 0, 0);

        uint256 amount = 100_000 * 10 ** dec;
        asset.mint(USER1, amount);
        vm.startPrank(USER1);
        asset.approve(address(vault), amount);
        vm.expectRevert(); // may revert if offset prevents minimum shares
        uint256 shares = vault.deposit(amount, USER1);
        vm.stopPrank();

        // If it succeeded, verify invariant
        if (shares > 0) {
            assertApproxEqRel(
...
// ── Fuzz: multi-user economic cycles ──

contract FuzzMultiUser is FuzzBase {
    function testFuzz_multiUserCycle(
        uint256 amount1,
        uint256 amount2,
        uint256 yieldAmt
    ) public {
        ...
        uint256 s1 = vault.deposit(amount1, USER1);
        vm.stopPrank();
        totalIn += amount1;
        ...
        uint256 s2 = vault.deposit(amount2, USER2);
        vm.stopPrank();
        totalIn += amount2;

        if (yieldAmt > 0) {
            asset.addYield(address(vault), yieldAmt);
        }

        uint256 totalOut;
        vm.prank(USER1);
        totalOut += vault.redeem(s1, USER1, USER1);
        vm.prank(USER2);
        totalOut += vault.redeem(s2, USER2, USER2);

        // Total out should be between totalIn and totalIn + yield
        if (yieldAmt > 0) {
            assertGt(totalOut, totalIn, "Yield distributed");
            assertLe(
                totalOut,
                totalIn + yieldAmt,
                "Total out bounded by totalIn + yield"
            );
        } else {
            assertLe(totalOut, totalIn, "No yield = no extra value");
        }
    }
}
