...
contract SUSDSConnectorAuditTest is Test {
    using SafeERC20 for IERC20;

    IERC20 asset;
    ERC4626Mock sUsds;
    SUSDSConnectorUnderTest connector;

    function setUp() public {
        asset = IERC20(address(new Mock20("USDS", "USDS", 18)));
        sUsds = new ERC4626Mock(asset, "Savings USDS", "sUSDS", 18);
        connector = new SUSDSConnectorUnderTest(address(sUsds));
    }
    ...
    function test_deposit() public {
        ...
        assertGt(sUsds.balanceOf(address(connector)), 0, "sUSDS shares minted");
    }
    ...
    function test_ssrRateChange() public {
        uint256 amount = 100_000 * 10 ** 18;
        Mock20(address(asset)).mint(address(this), amount);
        asset.approve(address(connector), amount);
        asset.safeTransfer(address(connector), amount);
        ...
        // Sky Savings Rate accumulates: sUSDS/USDS rate increases
        sUsds.setExchangeRate(1.12e18); // 12% yield
    ...
    function test_partialWithdraw() public {
        ...
        uint256 pre = asset.balanceOf(address(this));
        connector.withdraw(IERC20(address(asset)), 30_000 * 10 ** 18);
        uint256 got = asset.balanceOf(address(this)) - pre;
        assertEq(got, 30_000 * 10 ** 18, "partial withdrawal");
    }
...
/// @notice Minimal sUSDS connector replica.
contract SUSDSConnectorUnderTest is IConnector {
    using SafeERC20 for IERC20;
    ...
    function totalAssets(IERC20) external view returns (uint256) {
        return
            ERC4626Mock(sUSDS).previewRedeem(
                ERC4626Mock(sUSDS).balanceOf(address(this))
            );
    }
    function deposit(IERC20 asset, uint256 amount) external {
        asset.forceApprove(sUSDS, amount);
        ERC4626Mock(sUSDS).deposit(amount, address(this));
    }
    function withdraw(IERC20 asset, uint256 amount) external {
        ERC4626Mock(sUSDS).withdraw(amount, address(this), address(this));
    ...
    function maxDeposit(IERC20) external view returns (uint256) {
        return ERC4626Mock(sUSDS).maxDeposit(address(this));
    }
    function maxWithdraw(IERC20) external view returns (uint256) {
        return ERC4626Mock(sUSDS).maxWithdraw(address(this));
    }
}
